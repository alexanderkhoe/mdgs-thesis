...
public class TestGlvqM {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		resumeWriter = new PrintWriter(new BufferedWriter(new FileWriter(
				...
				TestGlvqM.class.getSimpleName(), false)));		
	}
	...
	public int[] iterationset = { 200};
	public double[] alphaset  = { /*0.1, 0.075, 0.05, 0.01,*/ 0.005/*, 0.001*/};
	public int MAX_ATTEMPT 	  = 2;
	
	public ConfusionMatrix test(MParam[] mps, Dataset codebook, Dataset testset, int numclass){
	...
	@Test
	public void testGlvqM() throws IOException{
		utils.header("Running testGlvq Mahalanobis");
		utils.log(writer, String.format("\n%s",dateFormat.format(new Date())));
		utils.log(writer, "TrainGlvq Mahalanobis");
		utils.log(resumeWriter, String.format("\n%s",dateFormat.format(new Date())));
		utils.log(resumeWriter, "TrainGlvq Mahalanobis");
		urut++;
		
		/*TrainLvq*/
		for(int dt=0;dt < NUM_DATA;dt++){
			if(dt < 5 ) continue;
			Dataset trainset = new Dataset(trainsets[dt]);
			MParam[] mps = TrainMGlvq.normalizeData(trainset); 
			...
			for(int a=0;a < alphaset.length;a++){
				for(int b=0;b < iterationset.length;b++){
					...
					for(int attempt=0;attempt < MAX_ATTEMPT;attempt++){
						...
						cm1 = test(mps, net.codebook, trainsets[dt], nclass[dt]);
						cm2 = test(mps, net.codebook, testsets[dt], nclass[dt]);
						cm3 = test(mps, net.codebook, testsetsOutlier[dt], nclass[dt]);

						//best codebook
						cm4 = test(mps, train.bestCodebook.codebook, trainsets[dt], nclass[dt]);
						cm5 = test(mps, train.bestCodebook.codebook, testsets[dt], nclass[dt]);
						cm6 = test(mps, train.bestCodebook.codebook, testsetsOutlier[dt], nclass[dt]);
...
//					return;
		...
		utils.log("TrainGlvq Mahalanobis done");
	}	
