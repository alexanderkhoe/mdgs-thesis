...
public class TestAntarDaubechies {
	...
	public static String[] types = {"db2", "db4", "db6", "db8"}; 
	...
	public static void setUpBeforeClass() throws Exception {
		...
		for(int i=0; i < NUM_DATA;i++){
			...
			//int pos = i * 4;
			datasets[i] 		= new Dataset(Parameter.DATA_DAUBECHIES[i]);
			datasets[i].load();
		}
			...
			for(int b=0;b < iterationset.length;b++){
				...
				for(int attempt=0;attempt < MAX_ATTEMPT;attempt++){
					...
					sb.append(String.format("%2d\t%d\t%s\t%2d\t%3d\t", 
							attempt+1, (dt/4)+1,types[dt%4], nclass, trainset.getMasterData().numFeatures));
				...
				sb2.append(String.format("%2s\t%d\t%s\t%2d\t%3d\t", "##",(dt/4)+1, types[dt%4], nclass, trainset.getMasterData().numFeatures));
		...
		for(int dt=0;dt < NUM_DATA;dt++){
			...
			for(int b=0;b < iterationset.length;b++){
				...
				for(int attempt=0;attempt < MAX_ATTEMPT;attempt++){
					...
					sb.append(String.format("%2d\t%d\t%s\t%2d\t%3d\t", 
							attempt+1, (dt/4)+1, types[dt%4], 6, trainset.getMasterData().numFeatures));
					sb.append(String.format("%7.4f\t%4d\t", 
							alphaset[a], iterationset[b]));
				...
				sb2.append(String.format("%2s\t%d\t%s\t%2d\t%3d\t", "##",(dt/4)+1, types[dt%4], 6, trainset.getMasterData().numFeatures));
				sb2.append(String.format("%7.4f\t%4d\t", 
									alphaset[a], iterationset[b]));
